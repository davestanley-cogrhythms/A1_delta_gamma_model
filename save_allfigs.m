...
%     if ~exist('currfigname'); currfigname = '3_single_comp_only_Mcurr'; end
    ...
    currfname = 'kr'; 
    currfigname = '54a_reduced_RS_noise';
...
%         print(gcf,'-dpng',fullfile(basepath,sp,savenames{i}))
    ...
    %%
    mycomment = ['Made RS cells less noisy in order to get more regular gamma entrainment. Increased injected current to compensate for reduced poisson input.'];
    currd = pwd;
    cd ..
    system('git add *');
