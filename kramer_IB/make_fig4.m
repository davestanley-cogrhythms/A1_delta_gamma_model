...
adjusted_plv = max(adjusted_plv(:, 1:(end - 1)) - repmat(adjusted_plv(:, end), 1, mrv_size(3) - 1), 0);
...
fig_name = sprintf('fig4a_Iapp%g', I_app);
...
selected_PPfreq = [0.25 3 6.5 9 12];
...
fig_name = sprintf('fig4b_Iapp%g_stim%g_%gHz_%gHz_%gHz_%gHz_%gHz', I_app, selected_PPstim, selected_PPfreq);
...
selected_PPfreq = [0.25 3 6 6.5 7 9 12]; % [0.25 .5:.5:12];
...
for c = 1:length(selected_PPfreq)
    ...
    plotyy(1:length(theta), theta_interp, 1:length(theta), rho)
    ...
    set(gca, 'XAxisLocation', 'origin', 'YColor', 'w')
    ...
    fig_name = sprintf('fig4b_Iapp%g_stim%g_%gHz_rose', I_app, selected_PPstim, selected_PPfreq(c));
