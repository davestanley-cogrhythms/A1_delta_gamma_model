...
vary = {'deepRS', 'FMPstim', [0 -.8:-.1:-1.2];... 0:-.2:-1;... % 
    ...
    'deepRS', 'STPstim', 0;...
    'deepRS', 'Inoise', .25;...
    'deepRS->deepFS', 'g_SYN', .3;...
    'deepFS->deepRS', 'g_SYN', 0.075;...
    'deepFS->deepRS', 'tauRx', 0.25;...
    'deepFS->deepRS', 'tauDx', 2.5;...
    'deepRS->deepFS', 'tauDx', 65;...
    'deepRS->deepFS', 'tauRx', 0.25;...
    'deepFS', 'stim', 0.9500;...
    'deepRS', 'gl', 0.8;...
    'deepRS', 'gKs', 0;...
...
plot_2_vars(data, 'deepRS_V', 'deepRS_iFMPulses_input', [], [6 12]*10^4, [6, 4], 'column', titles)

saveas(gcf, [name, '_6to12s.fig'])

for s = 1:length(stims)
    
    plot_2_vars(data(FMPstim == stims(s)), 'deepRS_V', 'deepRS_iFMPulses_input', [], [], [4, 1], [], freq_titles)
...
fig_name = sprintf('fig4c_Iapp%g_stim%g_%gHz_%gHz_%gHz_%gHz_%gto%g', I_app,...
...
plot(1:size(adjusted_plv, 1), flipud(adjusted_plv), 'LineWidth', 1)
...
plot(1:size(adjusted_plv, 1), nanmean(flipud(adjusted_plv), 2), 'k', 'LineWidth', 2)

set(gca, 'XTick', 1:4, 'XTickLabel', {'.5-14 Hz', '1-13 Hz', '2-11 Hz', '3-9 Hz'})

axis tight

ylim([0 1])

fig_name = sprintf('fig4c_Iapp%g', I_app);
...
for c = 1:length(selected_FMPhighfreq)
    ...
    fig_name = sprintf('fig4c_Iapp%g_stim%g_%g-%gHz_rose', I_app, selected_FMPstim, selected_FMPlowfreq(c), selected_FMPhighfreq(c));
