...
%                 'figheight',chosen_height}; parallel_plot_entries{i} = [parallel_plot_entries{i} savefigure_options];
    ...
    if include_IB && include_NG && (include_FS || include_dFS5) && length(data) > 1
        ...
        parallel_plot_entries{i} = {@dsPlot2_PPStim, data, 'num_embedded_subplots', NESP, 'plot_type','raster','population','IB','plot_handle',@xp_raster1_GABAB,'variable','/V|THALL_GABA_gTH|GABAall_gTH|GABAA_gTH/','force_last','variables','Ndims_per_subplot',2,'plot_options',myplot_options,...
            'saved_fignum',i,'save_figname_prefix',['Fig ' num2str(i)],...
            'figheight',chosen_height}; parallel_plot_entries{i} = [parallel_plot_entries{i} savefigure_options];
        clear myplot_options
