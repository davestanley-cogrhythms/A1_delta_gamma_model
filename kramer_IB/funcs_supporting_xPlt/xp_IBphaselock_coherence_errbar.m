...
function hxp = xp_IBphaselock_coherence_errbar (xp, op)
    % xp must be 1x1 (e.g. 0 dimensional)
    % Produces phase locking by taking the coherence between the input
    % pulestrain and the GABA A/B conductance response. Plots the coherence
    % values at the frequency corresponding to the stimulus frequency.
    % This method:
    %   - first z-scores the data (although this is only for visualization)
    %   - DOES NOT detrend the data (although we could... just didn't implement it)
...
%         pulsetrain = data(i).IB_iPoissonNested_S2;
        synaptic_potential = data(i).IB_V(:,2);
        pulsetrain = data(i).IB_V(:,1);
...
%           trialave (average over trials when 1, don't average when 0) - optional. Default 0
        ...
        mygcf = gcf;
        plot_spectra_on = true;
        if plot_spectra_on
            
            figure('Position',[  680          32         663        1063]);
            subplot(411); plot(t,[synaptic_potential(:) pulsetrain(:)]); title('Raw');
            subplot(412); plot(f,S1); xlim([0 15]); ylim([0 0.8]); hold on; plot(f(ind),S1(ind),'kx'); title('Power pulsetrrain');
            subplot(413); plot(f,S2); xlim([0 15]); ylim([0 0.8]); hold on; plot(f(ind),S2(ind),'kx'); title('Power synaptic potential');
            subplot(414);plot(f,C); xlim([0 15]); hold on; plot(f(ind),C(ind),'kx'); title('Coherence');
        end
...
% %         NFFT = [];
%         i
%         synaptic_potential=detrend(synaptic_potential); % detrend the data
% %         [tmpPxx,f] = pmtm(synaptic_potential, params.tapers(1), NFFT, Fs); % calculate power
%         [tmpPxx,f,pxxc] = pmtm(synaptic_potential,[], NFFT, Fs,'unity','ConfidenceLevel',0.95); % calculate power
...
% %         r_all(i) = R(1,2);
% %         p_all(i) = P(1,2);
%         rlo_all(i) = pxxc(ind,1);
%         rup_all(i) = pxxc(ind,2);
      
    end
    
%     errY = cat(3,rlo_all,rup_all);
%     figure
%     hxp.hcurr = barwitherr(errY,r_all,'b');
    if plot_spectra_on; figure; end
    hxp.hcurr = bar(r_all,'FaceColor',[0.5,0.5,0.5]);
%     ylim([0,1.2]);
