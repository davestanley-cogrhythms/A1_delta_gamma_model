...
fields = [1 2; 2 3; 1 3; 1 3; 1 3];

x_labels = {'$I_{m}$ Activation', '$I_{inh}$ Activation', '$I_{m}$ Activation', '$I_{m}$ Activation', '$\Delta$ ($I_{m}$ Activation)'};

y_labels = {'$I_{inh}$ Activation', '$I_{K_{SS}}$ Activation', '$I_{K_{SS}}$ Activation', '$\Delta$ ($I_{m}$ Activation)', '$\Delta^2$ ($I_{m}$ Activation)'};

no_panels = size(fields, 1);

figure

sims = [5]; no_sims = length(sims);
...
for s = 1:no_sims
    ...
    for f = 1:no_panels
        ...
        subplot(no_sims, no_panels, (s - 1)*no_panels + f)
        
        activation_1 = [data(stim_indices).(data_field{fields(f, 1)})];
        
        if f ~= 4 && f ~= 5
            
            activation_2 = [data(stim_indices).(data_field{fields(f, 2)})];
        ...
        xlabel(x_labels{f}, 'interpreter', 'Latex')
        
        ylabel(y_labels{f}, 'interpreter', 'Latex')
        ...
        elseif f == 2 || f == 3
            ...
            [poly, stats] = polyfit(activation_1(all_pre_spike_indicator)', activation_2(all_pre_spike_indicator)', 2);
            
            x_lims = xlim; x_vals = linspace(x_lims(1), x_lims(2), 100);
            
            rline = poly(1)*x_vals.^2 + poly(2)*x_vals + poly(3);
        ...
        %         if f == 3
        %
        %             legend(h(1:3), {'No Input Pulse'; 'Input Pulse'; 'First Post-Input Spike'}, 'Location', 'SouthEast')
        %
        %         else
        if f == 1
...
set(gcf, 'Units', 'inches', 'Position', 1 + [0 0 14 3], 'PaperUnits', 'inches', 'PaperPosition', 1 + [0 0 14 3])
    
saveas(gcf, 'STP_analysis_MIS.fig')
