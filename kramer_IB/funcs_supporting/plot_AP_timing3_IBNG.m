...
function h = plot_AP_timing3_IBNG(data,ind_range)
    ...
    hold on; plot(data.time(ind),data.IB_iPeriodicPulses_Iext(ind,:),'k','LineWidth',2)
    hold on; plot(data.time(ind),data.NG_GABA_gTH(ind,:)*20-6,'b','LineWidth',2)
    %xlim([1440,1560])
