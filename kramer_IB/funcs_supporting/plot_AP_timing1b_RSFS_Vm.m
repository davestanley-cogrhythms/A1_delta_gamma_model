...
%     i=i+1; hold on; h{i} = plot(data.time(ind),data.FS_V(ind,:)+0,'r');
    ...
    i=i+1; hold on; h{i} = plot(data.time(ind),mean(data.LTS_FS_IBaIBdbiSYNseed_s(ind,:),2)*100-150,'LineWidth',5);
    i=i+1; hold on; h{i} = plot(data.time(ind),mean(data.LTS_RS_IBaIBdbiSYNseed_s(ind,:),2)*100-150,'LineWidth',5);
    %i=i+1; hold on; h{i} = plot(data.time(ind),data.NG_GABA_gTH(ind,:)*20-6,'b','LineWidth',2);
    
    i=i+1; hold on; h{i} = plot(data.time(ind),data.RS_iPeriodicPulsesFacilitate_Iext(ind,1)*1-160,'k','LineWidth',5);
    %xlim([1440,1560])
